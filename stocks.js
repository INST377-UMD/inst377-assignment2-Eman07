...
async function fetchStockData(ticker, days) {
    ...
    // Construct API URL for aggregates endpoint with all required parameters
    const apiUrl = `https://api.polygon.io/v2/aggs/ticker/${ticker}/range/1/day/${formattedStartDate}/${formattedEndDate}?adjusted=true&sort=asc&limit=120&apiKey=${API_KEY}`;
    ...
    try {
        ...
        // Check if we have results and status is OK
        if (data.status === 'OK' && data.results && data.results.length > 0) {
            ...
            console.error("No data found or API error:", data);
            
            // Try fetching the previous day's data as a fallback
            try {
                const prevDayUrl = `https://api.polygon.io/v2/aggs/ticker/${ticker}/prev?adjusted=true&apiKey=${API_KEY}`;
                
                const prevResponse = await fetch(prevDayUrl);
                const prevData = await prevResponse.json();
                
                if (prevData.status === 'OK' && prevData.results && prevData.results.length > 0) {
                    // If we have previous day data, the ticker exists but no data for the range
                    alert(`Ticker ${ticker} exists, but no data available for the selected time range. Try a different time range.`);
                } else {
                    // If no previous day data either, perhaps the ticker doesn't exist
                    alert(`No data found for ticker ${ticker}. Please check the symbol and try again.`);
                }
            } catch (prevError) {
                console.error("Error fetching previous day data:", prevError);
                alert(`No data found for ${ticker}. Please check the ticker symbol and try again.`);
            }
            
            // Create empty chart
            createEmptyChart(ticker);
        }
    } catch (error) {
        console.error('Error fetching stock data:', error);
        alert('Failed to fetch stock data. Please make sure the CORS extension is enabled and try again.');
    } finally {
...
function createEmptyChart(ticker) {
    // Destroy existing chart if it exists
    if (chartInstance) {
        chartInstance.destroy();
    }
    ...
    chartInstance = new Chart(ctx, {
        ...
        options: {
            ...
            plugins: {
                title: {
                    display: true,
                    text: `No data available for ${ticker}`,
                    font: {
                        size: 16
                    }
                }
...
async function fetchTopRedditStocks() {
    try {
        // Fetch data from the API with the specific date parameter
